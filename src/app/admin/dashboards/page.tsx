...
const Reportes: React.FC = () => {
  const [dataExistencias, setDataExistencias] = useState<
    VmReporteExistencias[]
  >([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const responseExistencias = await fetch(
          "http://localhost:5239/api/Reporte/ReporteExistencias"
        );
        const existencias = await responseExistencias.json();
        setDataExistencias(existencias);

        const responsePedidos = await fetch(
          "http://localhost:5239/api/Reporte/ReportePedidos"
        );
        ...
        <LayoutDivComponent>
          ...
          <div className="text-left">
            ...
            <div className="mb-6 flex space-x-4">
              {" "}
              {/* Cambié justify-between por space-x-4 para controlar el espacio */}
            ...
            {/* Agrega un divisor entre los gráficos */}
            <div className="mb-12">
              <div className="w-full mx-auto">
                {" "}
                {/* Cambia de md:w-3/4 lg:w-1/2 a w-full */}
